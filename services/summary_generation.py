...
def get_summary(query, vectorstore):
    # Retrieve the QA chat prompt template from the LangChain hub
    retrieval_qa_chat_prompt = hub.pull("langchain-ai/retrieval-qa-chat")

    # Initialize a document combination chain with a Chat model (using Google Generative AI)
    combine_docs_chain = create_stuff_documents_chain(
        ChatGoogleGenerativeAI(model="gemini-1.5-flash-latest", temperature=0),  # Specify the model
        retrieval_qa_chat_prompt  # Pass the retrieved prompt
    )
